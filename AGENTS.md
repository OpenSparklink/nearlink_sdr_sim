...
| 6.2.1.4 | UWB 脉冲波形与芯片调制 | 完成 | `phy/uwb_pulse.py` |
| 6.7 | 窄带跳频测量帧传输 (参数/调度/测量量) | 完成 | `phy/measurement_tx.py` |
| 6.8 | 超宽带脉冲测量帧传输 (参数/调度/测量量) | 完成 | `phy/measurement_tx.py` |
...
| 6.6 | 系统管理帧编解码 | 完成 | `mac/smf.py` |
| 6.6.2.1 | 系统管理帧调度信令 | 完成 | `mac/smf.py` |
| 6.6.2.2 | 链路信令 | 完成 | `mac/smf.py` |
| 6.6.2.3 | 偏移信令 | 完成 | `mac/smf.py` |
| 7.1.4.10 | 非链接态窄带跳频测量配置 | 完成 | `mac/broadcast.py` |
| 7.1.4.11 | 非链接态超宽带脉冲测量配置 | 完成 | `mac/broadcast.py` |
...
| - | SleNode 集成仿真 (跳频/接入/扫频/功率/测量) | 完成 | `sim/link_sim.py` (Phase 15) |
| - | 多用户干扰 + Doppler 时变信道仿真 | 完成 | `sim/link_sim.py` (Phase 16), `phy/channel.py` |
...
| 7.1.7 | 广播帧控制交互 (角色协商+非链接态广播) | 完成 | `mac/access.py` |
| 7.1.2 | 发现流程 | 完成 | `mac/access.py` |
...
| 6.5.2.5 | 事件组集合 | 完成 | `phy/data_link.py` |
| 6.5.3 | 同步业务适配 (周期/非周期) | 完成 | `phy/data_link.py` |
